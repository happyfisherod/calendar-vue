export default {
  install: function() {
     ...
     * Github: https://github.com/itsalb3rt/ligne_paginatejs
     **/
    ...
    function lignePaginate() {
      ...
      var hiddenPaginateControls = function() {
        document.querySelector(".paginate_controls").style.visibility =
          "hidden";
      };
      ...
      var pageButtons = function(numberOfPage, currentPage) {
        ...
        let nodes = Array.from(
          document.querySelector(".thead-table tr").children
        );
        let sortel = document.querySelector(".asort,.dsort");
        let indx = 1;
        let sortby = '"asc"';
        let coltype = "";
        if (sortel) {
          coltype = '"' + sortel.getAttribute("data-type") + '"';
          indx = nodes.indexOf(sortel) + 1;
          sortby = sortel.classList.contains("asort") ? '"asc"' : '"desc"';
        }
        let buttons =
          "<input type='button' value='← prev' class='paginate_control_prev' onclick='paginate.sort(" +
          (currentPage - 1) +
          "," +
          coltype +
          "," +
        ...
        let nextButton =
          "<input type='button' value='next →' class='paginate_control_next' onclick='paginate.sort(" +
          (currentPage + 1) +
          "," +
          coltype +
          "," +
      ...
      _lignePaginate.sort = function(
        ...
        let ttr = tr.sort(function(a, b) {
          let r1 = new DOMParser().parseFromString(a, "text/xml");
          let r1t = r1.querySelector("td:nth-child(" + sortnum + ")").innerHTML;

          let r2 = new DOMParser().parseFromString(b, "text/xml");
          let r2t = r2.querySelector("td:nth-child(" + sortnum + ")").innerHTML;
          ...
          if (sortby == "asc") {
            if (coltype == "text") {
              ...
              let split1 = r1t.split("/");
              let split2 = r2t.split("/");
              return (
                new Date(split1[2] + "-" + split1[1] + "-" + split1[0]) -
                new Date(split2[2] + "-" + split2[1] + "-" + split2[0])
              );
            }
          } else {
            if (coltype == "text") {
        ...
        let rows = "",
          startPoint =
            settings.numberPerPage * selectedPageNumber -
            settings.numberPerPage;
        for (
          let $i = startPoint;
          $i < startPoint + settings.numberPerPage && $i < ttr.length;
          $i++
        ) {
          rows += ttr[$i];
        }
